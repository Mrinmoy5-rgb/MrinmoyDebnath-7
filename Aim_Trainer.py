...
def draw(win, targets, elapsed_time, target_pressed, misses):
    win.fill(BG_COLOR)
...
def end_screen(win, elapsed_time, target_pressed, clicks):
    ...
    title = title_font.render("GAME OVER", 1, "white")
    time_label = LABEL_FONT.render(f"Time: {format_time(elapsed_time)}", 1, "white")
    speed_label = LABEL_FONT.render(f"Speed: {speed} t/s", 1, "white")
    hits_label = LABEL_FONT.render(f"Hits: {target_pressed}", 1, "white")
    accuracy_label = LABEL_FONT.render(f"Accuracy: {accuracy}%", 1, "white")
    restart_label = LABEL_FONT.render("Press any key to close", 1, "white")

    win.blit(title, (WIDTH/2 - title.get_width()/2, 100))
    win.blit(time_label, (WIDTH/2 - time_label.get_width()/2, 200))
    win.blit(speed_label, (WIDTH/2 - speed_label.get_width()/2, 250))
...
def main():
    ...
    while run:
        ...
        draw(WIN, targets, elapsed_time, target_pressed, misses)
        pygame.display.update()
